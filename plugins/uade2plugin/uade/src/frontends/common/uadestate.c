...
static int64_t samples_to_offset(int samples,
				 const struct uade_state *state)
{
	return ((int64_t) samples) * UADE_BYTES_PER_FRAME;
}

static int seek_subsong_relative(int samples, int subsong,
				 struct uade_state *state)
{
...
static int seek_song_relative(int samples, struct uade_state *state)
{
	int64_t seekoffs = samples_to_offset(samples, state);
	if (seekoffs < 0) {
...
int uade_seek(enum uade_seek_mode whence, double seconds, int subsong,
	      struct uade_state *state)
{
	size_t max_int = ((size_t) -1) / 2;
	const double limit = max_int / uade_get_sampling_rate(state);
	int samples;
...
int uade_seek_samples(enum uade_seek_mode whence, int samples, int subsong,
		      struct uade_state *state)
{
	switch (whence) {
	case UADE_SEEK_SONG_RELATIVE:
...
int uade_read(void *_data, size_t bytes, struct uade_state *state)
{
	uint8_t *data = _data;
	size_t copied = 0;

...
public:
	...
	HEPlayer(const std::string &fileName/*, const std::string &biosFileName */) {
		...
		if (psf_version == 1)
		{
			if ( psf_load( temp, &psf_file_system, psf_version, psf1_load, &lstate, psf1_info, &lstate ) < 0 )
			{
		...
		if (psf_version == 2)
		{
			...
			if ( psf_load( temp, &psf_file_system, psf_version, psf2fs_load_callback, psf2fs, psf1_info, &lstate ) < 0 )
			{
				throw player_exception();
			}
...
static const set<string> supported_ext { "psf", "psf2", "minipsf" , "minipsf2" };

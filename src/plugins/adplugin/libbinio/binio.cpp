...
binistream::Float binistream::ieee_single2float(Byte *data)
{
  ...
  if(!exp)	// Unnormalized float values
    return sign * pow(2.0, -126.0) * fract * pow(2.0, -23.0);
  else		// Normalized float values
    return sign * pow(2.0, exp - 127.0) * (fract * pow(2.0, -23.0) + 1);
...
binistream::Float binistream::ieee_double2float(Byte *data)
{
  ...
  unsigned int	exp = ((unsigned int)(data[0] & 0x7f) << 4) | (data[1] >> 4),
    fracthi4 = data[1] & 0xf;
  Float		fract = fracthi4 * pow(2.0, 48.0) + data[2] * pow(2.0, 40.0) + data[3] *
    pow(2.0, 32.0) + data[4] * pow(2.0, 24.0) + data[5] * pow(2.0, 16.0) + data[6] *
    pow(2.0, 8.0) + data[7];
  ...
  if(!exp)	// Unnormalized float values
    return sign * pow(2.0, -1022.0) * fract * pow(2.0, -52.0);
  else		// Normalized float values
    return sign * pow(2.0, exp - 1023.0) * (fract * pow(2.0, -52.0) + 1);

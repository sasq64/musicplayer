...
public:
    ...
    explicit MP3Player(const std::string& fileName)
    {
        ...
        setMeta("format", "MP3");
        if (dec.info.channels != channels) {
            channels = dec.info.channels;
            setMeta("channels", channels);
        }
        if (dec.info.hz != hz) {
            hz = dec.info.hz;
            setMeta("sample_rate", hz);
        }
    }

    ~MP3Player() override { mp3dec_ex_close(&dec); }
    std::shared_ptr<utils::Fifo<uint8_t>> fifo;

    int getSamples(int16_t* target, int noSamples) override
    {
        mp3dec_ex_read(&dec, (mp3d_sample_t*)target,
                       2 * noSamples / sizeof(mp3d_sample_t));
        if (dec.info.channels != channels) {
            channels = dec.info.channels;
            setMeta("channels", channels);
        }
